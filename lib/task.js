...
function createCustomizedNatsConnection() {
...
createCustomizedNatsConnection()
...
module.exports = {
    ...
    nats.subscribe(queue, function (particle, replyTo, subject) {
      ...
      Promise.resolve(processor.call(processorScope, particle, subject)).then((res) => {
        ...
        if (_.isUndefined(res)) {
          winston.info(`[NEO:TASK] Result for task "${queue}" was undefined. Skipping reply.`)
          return
        }
  ...
  connect: createCustomizedNatsConnection,
  disconnect: () => {
    nats.close()
    winston.info('[NEO:TASK] Close')
  }
}
